...
import { AnalysisPayload, AnalysisRequestCredentials, ExpectedPaginationData } from "../types/reqPayload.types.js";
import { analyzeApiResponse, makeAxiosRequest } from "../services/analysis.services.js";
import { AnalysisDetailSchema, InitialDbAnalysisDetailSchema, LazyLoadedAnalysisDetailData, PaginationData, TypedExpectedData } from "../types/resPayload.types.js";
import { deleteOperation, readOperation, writeOperation } from "../services/cache.services.js";
import { Analysis, IAnalysisQuery } from "../models/Analysis.js";
...
*/
export const newAnalysis = async (req: Request, res: ConfiguredResponse): Promise<ConfiguredResponse> => {
    const userId = ((req as ConfiguredRequest).accessTokenPayload).userId;
    const analysisRequest = (req as ConfiguredRequest).data as AnalysisRequestCredentials;
    ...
    try {
        ...
        if (now > user.creditRefillDate) {
            ...
            const updated = await User.findByIdAndUpdate(
                ...
                { new: true }
            ).lean<IUserQuery>().select("_id fullname email avatar credits plan role createdAt updatedAt enableAiAnalysis creditRefillDate");

            if (updated) writeOperation(`user:${updated._id.toString()}`, getUserProfilePayload(updated));
        ...
        const typedApiResponseData = apiFetchResult.data as TypedExpectedData;
...
/** 
 ...
 @returns {Promise<ConfiguredResponse>}
*/
export const saveAnalysis = async (req: Request, res: ConfiguredResponse): Promise<ConfiguredResponse> => {
    // ** Extract the users id attached to request ** \\
    const userId = (req as ConfiguredRequest).accessTokenPayload.userId;

    // ** Extract details about the analysis attached to request as well ** \\
    const analysisDetails = (req as ConfiguredRequest).data as AnalysisPayload;
    try {
...
/** 
 ...
 @returns {Promise<ConfiguredResponse>}
*/
export const fetchSavedAnalysisDetails = async (req: Request, res: ConfiguredResponse): Promise<ConfiguredResponse> => {
    // ** Extract the users id attached to request ** \\
    const userId = (req as ConfiguredRequest).accessTokenPayload.userId;
    ...
    const {
        ...
        method,
        status
    } = (req as ConfiguredRequest).data as ExpectedPaginationData;
    ...
    // ** Query ** \\
    let dbQuery: Record<any, any> = { user: userId };

    // ** Object being sent to frontend *8 \\
    let paginationData: PaginationData<LazyLoadedAnalysisDetailData[]> = {
        offset,
        page,
        limit,
        totalItems: 0,
    ...
    try {
        ...
        if (!cachedDataChunk) {
            ...
            // ** Data chunk ** \\
            const savedAnalysisDataChunk: IAnalysisQuery[] = await Analysis.find(dbQuery)
                .lean<IAnalysisQuery[]>()
            ...
            // ** Typed data chunk ** \\
            const typedSavedAnalysisDataChunk = savedAnalysisDataChunk as unknown as InitialDbAnalysisDetailSchema[];

            // ** Formatted chunk ** \\
            const formattedSavedAnalysisDataChunk: LazyLoadedAnalysisDetailData[] = typedSavedAnalysisDataChunk.map((savedAnalysis) => {
                const stringifiedId = (savedAnalysis?._id as mongoose.Types.ObjectId)?.toString();
                return {
                    _id: stringifiedId,
            ...
            // ** Cache hit ** \\
            const parsedCachedDataChunk: PaginationData<LazyLoadedAnalysisDetailData[]> = JSON.parse(cachedDataChunk);
...
/** 
 ...
 @returns {Promise<ConfiguredResponse>}
*/
export const getSingleAnalysisDetails = async (req: Request, res: ConfiguredResponse): Promise<ConfiguredResponse> => {
    // ** Extract the users id attached to request ** \\
    const userId = (req as ConfiguredRequest).accessTokenPayload.userId;

    //  ** Extract validated id attached to request ** \\
    const singleAnalysisDetailsId = ((req as ConfiguredRequest).data as { analysisId: string }).analysisId;
    ...
    try {
        if (!cachedAnalysisDetails) {
            ...
            const analysisBeingRequested: IAnalysisQuery = await Analysis.findOne({
            ...
            const typedAnalysis = analysisBeingRequested as unknown as InitialDbAnalysisDetailSchema;
            const formattedAnalysis: AnalysisDetailSchema = {
                ...(typedAnalysis || {}),
            ...
            // ** Cache hit ** \\
            const parsedAnalysisDetails: AnalysisDetailSchema = JSON.parse(cachedAnalysisDetails);
...
/** 
 ...
 @returns {Promise<ConfiguredResponse>}
*/
export const deleteSingleAnalysisDetails = async (req: Request, res: ConfiguredResponse): Promise<ConfiguredResponse> => {
    // ** Extract the users id attached to request ** \\
    const userId = (req as ConfiguredRequest).accessTokenPayload.userId;
    ...
    try {
        ...
        const analysisBeingDeleted: IAnalysisQuery = await Analysis.findOneAndDelete({
            _id: singleAnalysisDetailsId,
            user: userId
        }).lean<IAnalysisQuery>()
            .select("_id request response aiAnalysis cost createdAt updatedAt");
...
/** 
 ...
 @returns {Promise<ConfiguredResponse>}
*/
export const disabledOrEnableAiAnalysis = async (
    req: Request,
    res: ConfiguredResponse
): Promise<ConfiguredResponse> => {
