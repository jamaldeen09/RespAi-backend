...
import { ConfiguredRequest, ConfiguredResponse } from "../types/api.types";
...
*/
export const rateLimit = (options: { key: "ip" | "user"; limit: number; windowMs: number; }) => {
    return async (req: Request, res: ConfiguredResponse, next: NextFunction) => {
        try {
            const now = Date.now();
            ...
            // ** Choose key type ** \\
            if (options.key === "ip") {
                identifier = req.ip || req.headers["x-forwarded-for"] as string || req.socket.remoteAddress as string;
