...
*/
export const validateToken = async (req, res, tokenType) => {
    try {
...
*/
export const createToken = (tokenType, payload) => {
    if (tokenType === "accessToken") {
...
export const cacheUser = (user) => {
    ...
    writeOperation(`user-${user._id}`, cacheData);
};
/**
  @param {IUserDocument} user - The user's document stored in the database
  @returns {void}
*/
export const getUserProfilePayload = (user) => {
    return {
        ...user,
        creditRefillDate: user.creditRefillDate.toISOString(),
        createdAt: user.createdAt.toISOString(),
