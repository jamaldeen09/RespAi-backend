...
export const signup = async (req, res) => {
    ...
    try {
        ...
        const newUser = await User.create({
            fullname: `${signupCredentials.firstname.charAt(0).toUpperCase() + signupCredentials.firstname.slice(1)} ${signupCredentials.lastname.charAt(0).toUpperCase() + signupCredentials.lastname.slice(1)}`,
...
export const refresh = async (req, res) => {
    ...
    try {
        // ** Checks if the user requesting to log out exists ** \\
        const user = await User.findById(userId).lean().select("_id fullname");
        if (!user)
            return res.status(404).json({
                success: false,
                message: "Account was not found",
